...
export type StackPeekDirection = "top" | "bottom" | "left" | "right";
...
const defaults: Required<
	...
	>
> &
	Pick<StackifyOptions, "orientation" | "size" | "stackDirection"> = {
	offset: 20,
	scaleStep: 0,
...
function applyDatasetOptions(container: HTMLElement, opts: ResolvedOpts): void {
	...
	container.dataset.orientation =
		opts.orientation ?? (opts.layout === "marquee" ? "horizontal" : "vertical");
	...
	if (opts.layout === "stack") {
		...
		container.dataset.stackDirection =
			opts.stackDirection ??
			((opts.orientation ?? "vertical") === "vertical" ? "top" : "left");
	}
}
...
function createEngine(
	...
	return opts.layout === "marquee"
		? createMarqueeEngine(container, opts)
		: createStackEngine(container, opts);
}
...
function createStackEngine(
	...
	const stackDirection: StackPeekDirection =
		opts.stackDirection ?? (vertical ? "top" : "left");
	...
	const cardRestore = new Map<HTMLElement, string>(
		original.map((card) => [card, card.style.cssText]),
	);
	...
	function applyPositions(): void {
		...
		order.forEach((card, i) => {
			...
			const peekDelta =
				opts.peekWidth === "expand"
					? i * opts.peekWidthStep
					: opts.peekWidth === "shrink"
						? -i * opts.peekWidthStep
						: 0;
			...
			if (peekScale !== 1) {
				transformParts.push(
					mainAxisVertical ? `scaleX(${peekScale})` : `scaleY(${peekScale})`,
				);
			}
			// Reset all four edges each run — direction (and thus which edge is
			// "main axis") can differ per card in `stackDirection: "all"`.
			card.style.top = "";
			card.style.right = "";
			card.style.bottom = "";
			card.style.left = "";
	...
	const onPointerDown = (e: PointerEvent) => {
		if (order[0] !== e.currentTarget && !order[0].contains(e.target as Node))
			return;
		dragging = true;
		dragStart = coord(e);
	...
	const onPointerMove = (e: PointerEvent) => {
		...
		order[0].style.transform = vertical
			? `translateY(${d}px)`
			: `translateX(${d}px)`;
	};
...
function createMarqueeEngine(
	...
	function setActiveClass(): void {
		...
		original.forEach((card, i) =>
			card.classList.toggle(opts.activeClass, i === idx),
		);
		clones.forEach((card, i) =>
			card.classList.toggle(opts.activeClass, i === idx),
		);
	}
	...
	const observer = new MutationObserver((mutations) => {
		...
		for (const mutation of mutations) {
			...
			for (const node of mutation.addedNodes) {
				if (
					node.nodeType === 1 &&
					!original.includes(node as HTMLElement) &&
					node !== track
				) {
...
function readDataOptions(container: HTMLElement): Partial<StackifyOptions> {
	...
	if (ds.orientation)
		opts.orientation = ds.orientation as "vertical" | "horizontal";
	if (ds.offset !== undefined) opts.offset = Number(ds.offset);
	if (ds.scaleStep !== undefined) opts.scaleStep = Number(ds.scaleStep);
	if (ds.visibleCards !== undefined)
		opts.visibleCards = Number(ds.visibleCards);
	...
	if (ds.peekWidth)
		opts.peekWidth = ds.peekWidth as "expand" | "shrink" | "none";
	if (ds.stackDirection)
		opts.stackDirection = ds.stackDirection as StackPeekDirection;
	if (ds.peekWidthStep !== undefined)
		opts.peekWidthStep = Number(ds.peekWidthStep);
	if (ds.marqueeSpeed !== undefined)
		opts.marqueeSpeed = Number(ds.marqueeSpeed);
	if (ds.autoplay !== undefined) opts.autoplay = ds.autoplay === "true";
	if (ds.pauseOnHover !== undefined)
		opts.pauseOnHover = ds.pauseOnHover === "true";
	if (ds.clickToActivate !== undefined)
		opts.clickToActivate = ds.clickToActivate === "true";
	...
	const engines = containers
		.map((container) => {
			...
			const merged = mergeOptions(
				mergeOptions(defaults, dataOpts),
				options || {},
			);
