...
Lazy-loads images (or background-images on any element) with
`IntersectionObserver` once they scroll near the viewport.
...
```ts
lazify("img[data-src]", { rootMargin: "200px", onLoad: (el) => console.log("loaded", el) });
```
